...
function Statistics() {
  ...
  const [infoMessage, setInfoMessage] = React.useState("Stats for " + Utility.getPaddedDate(new Date()));
  const handleSetters = {}
  ...
  const groupByRoute = (d)=>{
    const NIP = d && d.filter(v=>(v && v.route && v.route.toLowerCase()==='nip'));
    const Direct = d && d.filter(v=>(v && v.route && v.route.toLowerCase()==='direct'));
    return [NIP, Direct ];
  }
